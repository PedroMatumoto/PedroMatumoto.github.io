...
const CardNav: React.FC<CardNavProps> = ({
  ...
  // handle clicks on links: if the href contains a hash (#id), scroll to that element
  const handleLinkClick = (
    ...
    if (targetEl) {
      ...
      if (tlRef.current && isExpanded) {
        // reverse the animation and collapse states
    ...
    >
      ...
      >
        <div className="card-nav-top absolute inset-x-0 top-0 z-[2] flex h-[60px] items-center justify-between p-2 pl-[1.1rem]">
          ...
          <button
            type="button"
            className="card-nav-cta-button hidden h-full cursor-pointer rounded-[calc(0.75rem-0.2rem)] border-0 px-4 font-medium leading-none transition-colors duration-300 md:inline-flex md:items-center md:justify-center"
            ...
            >
              ...
              <div className="nav-card-links mt-auto flex flex-col gap-[2px]">
                {item.links?.map((lnk, i) => {
                  const hasHash = lnk.href && lnk.href.indexOf('#') !== -1
                  const external = /^(https?:|mailto:)/.test(lnk.href)
